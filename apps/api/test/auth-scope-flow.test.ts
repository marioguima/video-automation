...
test("auth scope flow: bootstrap -> me/context -> login keeps same workspace", async () => {
  ...
  const dbBeforePasswordChange = new Database(runtime.dbPath, { readonly: true });
  let previousPasswordHash: string;
  try {
    const row = dbBeforePasswordChange
      .prepare('SELECT passwordHash FROM "User" WHERE id = ? LIMIT 1')
      .get(meBody.user.id) as { passwordHash: string } | undefined;
    assert.ok(row, "User should exist before password change");
    previousPasswordHash = row.passwordHash;
  } finally {
    dbBeforePasswordChange.close();
  }

  // 6) Troca a senha via endpoint autenticado e confirma que o hash mudou no banco.
  ...
  assert.deepEqual(changePasswordRes.json(), { ok: true });

  const dbAfterPasswordChange = new Database(runtime.dbPath, { readonly: true });
  try {
    const row = dbAfterPasswordChange
      .prepare('SELECT passwordHash FROM "User" WHERE id = ? LIMIT 1')
      .get(meBody.user.id) as { passwordHash: string } | undefined;
    assert.ok(row, "User should exist after password change");
    assert.notEqual(row.passwordHash, previousPasswordHash);
  } finally {
    dbAfterPasswordChange.close();
  }

  // 7) A senha antiga falha e a nova autentica.
  const oldPasswordLoginRes = await app.inject({
    method: "POST",
    url: "/auth/login",
    payload: { email, password }
  });
  assert.equal(oldPasswordLoginRes.statusCode, 401);
