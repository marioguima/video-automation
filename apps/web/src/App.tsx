...
const App: React.FC = () => {
  ...
  const [pendingContentDraft, setPendingContentDraft] = useState<ContentQuickStartDraft | null>(null);
  const [pendingContentProjectId, setPendingContentProjectId] = useState<string | null>(null);
  ...
  const handleImportContentDraft = (draft: { title?: string; sourceText: string }) => {
    setPendingContentDraft({
      title: draft.title,
      sourceText: draft.sourceText,
      nonce: Date.now()
    });
    setCurrentView('quick-content');
  };

  const handleOpenContentProject = (projectId: string) => {
    setPendingContentProjectId(projectId);
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'dashboard':
        return (
          <Dashboard
            ...
            onImportContent={handleImportContentDraft}
            onOpenContentProject={handleOpenContentProject}
          />
        );
      ...
      case 'quick-content':
        return (
          <ContentQuickStart
            initialDraft={pendingContentDraft}
            onInitialDraftConsumed={() => setPendingContentDraft(null)}
          />
        );
      case 'course-editor':
        return (
