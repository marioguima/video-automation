...
import { DEFAULT_PROJECT_DESTINATIONS, formatDestination, type Destination } from '../lib/contentDestinations';
import ContentImportCard from './dashboard/ContentImportCard';
...
interface DashboardProps {
  ...
  homeView?: Extract<ViewType, 'dashboard' | 'course-dashboard'>;
  onImportContent?: (draft: { title?: string; sourceText: string }) => void;
  onOpenContentProject?: (projectId: string) => void;
}
...
function getProjectDestinations(project: ContentProject): Destination[] {
...
function getProjectAspectRatios(project: ContentProject): AspectRatio[] {
...
function getProjectOutputs(project: ContentProject): ProjectOutput[] {
...
function getItemStage(item: ContentItem): ProductionStage {
  ...
  return (item.status as ProductionStage) || 'script';
}

function getProjectProgress(items: ContentItem[]): number {
  ...
  const total = items.reduce((sum, item) => sum + (stageWeight[getItemStage(item)] ?? 18), 0);
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const mapProject = (project: ContentProject, items: ContentItem[]) => {
    const destinations = getProjectDestinations(project);
    const outputs = getProjectOutputs(project);
    const outputCount =
      outputs.length > 0 ? outputs.length : destinations.length * getProjectAspectRatios(project).length;
    const pendingItems = items.filter((item) => ['idea', 'script', 'scheduled'].includes(getItemStage(item))).length;
    const runningItems = items.filter((item) => ['scenes', 'assets', 'editing'].includes(getItemStage(item))).length;
    const destinationLabel = destinations.length > 0
      ? destinations.slice(0, 2).map(formatDestination).join(', ')
      : 'Project';
    const latestItemTime = items.reduce((latest, item) => Math.max(latest, getUpdatedAt(item)), 0);
    const latest = Math.max(getUpdatedAt(project), latestItemTime);
    return {
      id: project.id,
      title: project.name,
    ...
    duration: '',
      lastUpdated: latest > 0 ? new Date(latest).toISOString() : project.createdAt,
    build: {
        progressPercent: getProjectProgress(items),
      jobs: {
        blocks: {
  ...
  const buildContentMetrics = useCallback(
    (projects: ContentProject[], items: ContentItem[], range: DashboardRangeKey): DashboardMetrics => {
      const now = new Date();
      const since = new Date(getRangeStartMs(range));
      const outputCount = projects.reduce((sum, project) => {
        const outputs = getProjectOutputs(project);
        return sum + (outputs.length > 0 ? outputs.length : getProjectDestinations(project).length * getProjectAspectRatios(project).length);
      }, 0);
      const scheduledCount = items.filter((item) => Boolean(item.metadata?.plannedPublishAt)).length;
      const createdInRange = (value?: string) => {
      ...
      return {
        range: {
          key: range,
          label: DASHBOARD_RANGE_LABEL_BY_KEY[range],
          since: since.toISOString(),
          until: now.toISOString()
        },
        totals: {
          courses: projects.length,
          lessons: items.length,
          contentSeconds: outputCount,
          storageUsedBytes: scheduledCount
        },
        growth: {
          courses: projects.filter((project) => createdInRange(project.createdAt)).length,
          lessons: items.filter((item) => createdInRange(item.createdAt)).length,
          contentSeconds: projects
            .filter((project) => createdInRange(project.createdAt))
            .reduce((sum, project) => {
              const outputs = getProjectOutputs(project);
              return sum + (outputs.length > 0 ? outputs.length : getProjectDestinations(project).length * getProjectAspectRatios(project).length);
            }, 0),
          storageUsedBytes: items.filter((item) => createdInRange(item.createdAt) && item.metadata?.plannedPublishAt).length
        },
        disk: {
          totalBytes: null,
          freeBytes: null,
          usedPercentOfDisk: items.length > 0 ? Math.round((scheduledCount / items.length) * 100) : 0,
          storageUsedPercentOfFree: items.length > 0 ? Math.round((scheduledCount / items.length) * 100) : 0
        }
      };
    },
    []
  ...
  useEffect(() => {
    const fetchRecent = async () => {
      setIsLoading(true);
      try {
        const projects = await apiGet<ContentProject[]>('/content-projects', { cacheMs: 0, dedupe: false });
        const projectRows = await Promise.all(
          projects.map(async (project) => {
            let items: ContentItem[] = [];
            try {
              items = await apiGet<ContentItem[]>(`/content-projects/${project.id}/items`, {
                cacheMs: 0,
                dedupe: false
              });
            } catch (err) {
              console.warn('Failed to load content items for project dashboard.', err);
            }
            const courseCreatedAt = getCreatedAt(project);
            const latest = Math.max(getUpdatedAt(project), ...items.map(getUpdatedAt));
            const nextCourse = mapProject(project, items);
            return { course: nextCourse, latest, courseCreatedAt, items };
          })
        );
        setContentProjects(projects);
        setContentItems(projectRows.flatMap((row) => row.items));
        const rankedSorted = projectRows.sort((a, b) => b.latest - a.latest);
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      setMetrics(buildContentMetrics(contentProjects, contentItems, metricsRange));
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoadingMetrics(false);
    }
  }, [buildContentMetrics, contentItems, contentProjects, metricsRange]);
  ...
  const openContentProject = (course: Course) => {
    if (onOpenContentProject) {
      onOpenContentProject(course.id);
      return;
    }
    setView('content');
  };
  ...
  const topicBaseCourses = recentCourses;
  ...
  const courseTopicChartData =
    courseTopicBreakdown.length > 0
      ...
      : [{ topic: 'empty', label: 'No projects', count: 1, percent: 0, fill: 'hsl(var(--muted))' }];
  const courseTopicConfig = useMemo<ChartConfig>(() => {
    const base: ChartConfig = { count: { label: 'Projects' } };
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <StatCard
                  ...
                  value={String(totalContentValue)}
                  trend={isLoadingMetrics ? '...' : `+${growthContentValue}`}
                  icon={Clock}
                  color="amber"
                />
                <StatCard
                  title="Scheduled Items"
                  value={String(totalStorageValue)}
                  trend={isLoadingMetrics ? '...' : `+${growthStorageValue}`}
                  icon={Database}
                  color="orange"
                  showProgress
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div ref={recentCoursesMenuRef} className="relative">
                    <Button
                      ...
                      aria-label="Filter recent projects range"
                    >
                      <Filter size={16} />
                    </Button>
                  ...
                  <Button variant="ghost" className="hidden sm:flex text-xs font-bold text-primary" onClick={() => setView('content')}>
                    VIEW ALL
                  </Button>
                </div>
              </div>
                      ...
                      {recentCoursesLimited.map((course) => {
                        ...
                        >
                          ...
                          <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                            <span className="font-semibold">{course.lessons} content items</span>
                      ...
                      <table className="w-full text-left text-sm">
                        <thead className="bg-muted/50">
                          <tr className="border-b">
                            ...
                            <th className="px-6 py-4 font-bold text-muted-foreground uppercase text-[10px]">Destinations</th>
                            <th className="px-6 py-4 font-bold text-muted-foreground uppercase text-[10px] text-center">Content</th>
                            ...
                            <th className="px-6 py-4 font-bold text-muted-foreground uppercase text-[10px] text-center">Status</th>
                        ...
                        <tbody className="divide-y">
                          {recentCoursesFiltered.map((course) => {
                            ...
                            <tr 
                              ...
                              onClick={() => openContentProject(course)}
                            >
                            ...
                            <div className="h-full">
                              <div className="pr-20 flex h-full min-w-0 flex-col">
                                ...
                                <p className="mt-auto text-xs text-muted-foreground font-medium">
                                  {course.lessons} content items • {course.build?.progressPercent ?? 0}% built
                                </p>
                              </div>
                            </div>
                            {hasBuildActivity(course) ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="bg-primary text-primary-foreground border-none relative overflow-hidden group flex flex-col justify-end min-h-[190px] transition-all hover:-translate-y-1 rounded-[5px]">
                <div className="relative z-10 p-6">
                  <h4 className="text-lg font-bold mb-2">Create New Project</h4>
                  <p className="text-primary-foreground/90 text-sm font-medium leading-relaxed max-w-sm mb-5">
                    Define destinations, formats, and default outputs for a content workspace.
                  </p>
                  <Button
              ...
              <ContentImportCard
                onImportContent={(draft) => {
                  if (onImportContent) {
                    onImportContent(draft);
                    return;
                  }
                  setView('quick-content');
                }}
              />
            </div>
          </div>
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            {/* Project Destination Donut Chart */}
            <div>
              <div className="flex min-h-9 items-center justify-between mb-1 px-1">
                 ...
                 <div ref={courseTopicMenuRef} className="relative">
                   <Button
                     ...
                     aria-label="Filter project destinations range"
                   >
                      <Filter size={16} />
                   </Button>
