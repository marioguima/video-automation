...
interface CourseVideoEditorProps {
...
type AnimationPromptEditorProps = {
  blockId: string;
  value: LessonBlock["animationPrompt"];
  onSave: (blockId: string, value: LessonBlock["animationPrompt"]) => void;
  disabled?: boolean;
  invalid?: boolean;
};

type DirectionNotesEditorProps = {
  blockId: string;
  value: LessonBlock["directionNotes"];
  onSave: (blockId: string, value: LessonBlock["directionNotes"]) => void;
  disabled?: boolean;
};

type SoundEffectPromptEditorProps = {
  blockId: string;
  value: LessonBlock["soundEffectPrompt"];
  onSave: (blockId: string, value: LessonBlock["soundEffectPrompt"]) => void;
  disabled?: boolean;
};
...
const AnimationPromptEditor: React.FC<AnimationPromptEditorProps> = React.memo(({ blockId, value, onSave, disabled = false, invalid = false }) => {
  const [draft, setDraft] = useState(value);
  const draftRef = useRef(value);
  const saveTimer = useRef<number | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  const scheduleSave = (nextValue: LessonBlock["animationPrompt"]) => {
    if (disabled) return;
    if (saveTimer.current) {
      window.clearTimeout(saveTimer.current);
    }
  ...
  const updateDraft = (updater: (prev: LessonBlock["animationPrompt"]) => LessonBlock["animationPrompt"]) => {
    if (disabled) return;
    setDraft((prev) => {
      const next = updater(prev);
      draftRef.current = next;
  ...
  return (
    <div className={`space-y-4 rounded-[7px] p-2 transition-colors ${disabled ? 'opacity-60' : ''} ${invalid ? 'bg-red-500/10 ring-1 ring-red-500/40' : ''}`} ref={containerRef}>
      <div className="space-y-1.5">
        <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Animation Prompt</label>
        <textarea
          className={`w-full p-3 bg-[hsl(var(--editor-input))] border rounded-[6px] text-xs text-foreground font-medium outline-none transition-all resize-none leading-relaxed h-20 ${
            invalid
              ? 'border-red-500/60 bg-red-500/10 focus:border-red-500/70'
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="space-y-1.5">
          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Motion</label>
          <input
            className="w-full bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs outline-none focus:border-primary/40 transition-all h-9 text-foreground px-3"
            value={draft.motion}
            onChange={(e) => updateDraft((prev) => ({ ...prev, motion: e.target.value }))}
            onFocus={handleFocus}
            onBlur={handleBlur}
            disabled={disabled}
          />
        ...
        <div className="space-y-1.5">
          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Camera</label>
          <input
            className="w-full bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs outline-none focus:border-primary/40 transition-all h-9 text-foreground px-3"
            value={draft.camera}
            onChange={(e) => updateDraft((prev) => ({ ...prev, camera: e.target.value }))}
            onFocus={handleFocus}
            onBlur={handleBlur}
            disabled={disabled}
          />
        ...
        <div className="space-y-1.5">
          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Duration</label>
          <input
            className="w-full bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs outline-none focus:border-primary/40 transition-all h-9 text-foreground px-3"
            value={draft.durationHint}
            onChange={(e) => updateDraft((prev) => ({ ...prev, durationHint: e.target.value }))}
            onFocus={handleFocus}
            onBlur={handleBlur}
            disabled={disabled}
          />
...
AnimationPromptEditor.displayName = "AnimationPromptEditor";

const DirectionNotesEditor: React.FC<DirectionNotesEditorProps> = React.memo(({ blockId, value, onSave, disabled = false }) => {
  const [draft, setDraft] = useState(value);
  const saveTimer = useRef<number | null>(null);
  ...
  return (
    <div className={`space-y-1.5 rounded-[7px] p-2 transition-colors ${disabled ? 'opacity-60' : ''}`}>
      <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Direction Notes</label>
      <textarea
        className="w-full p-3 bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs text-foreground font-medium outline-none focus:border-primary/40 transition-all resize-none leading-relaxed h-20"
        value={draft.notes}
        onChange={(event) => updateNotes(event.target.value)}
        onBlur={() => onSave(blockId, draft)}
        disabled={disabled}
      />
    </div>
  );
...
DirectionNotesEditor.displayName = "DirectionNotesEditor";

const SoundEffectPromptEditor: React.FC<SoundEffectPromptEditorProps> = React.memo(({ blockId, value, onSave, disabled = false }) => {
  const [draft, setDraft] = useState(value);
  const saveTimer = useRef<number | null>(null);
  ...
  const updateDraft = (updater: (prev: LessonBlock["soundEffectPrompt"]) => LessonBlock["soundEffectPrompt"]) => {
    if (disabled) return;
    setDraft((prev) => {
      const next = updater(prev);
      if (saveTimer.current) window.clearTimeout(saveTimer.current);
  ...
  return (
    <div className={`space-y-4 rounded-[7px] p-2 transition-colors ${disabled ? 'opacity-60' : ''}`}>
      <div className="space-y-1.5">
        <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Sound Effect Prompt</label>
        <textarea
          className="w-full p-3 bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs text-foreground font-medium outline-none focus:border-primary/40 transition-all resize-none leading-relaxed h-16"
          value={draft.prompt}
          onChange={(event) => updateDraft((prev) => ({ ...prev, prompt: event.target.value }))}
          onBlur={() => onSave(blockId, draft)}
          disabled={disabled}
        />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-tight">Timing</label>
          <input
            className="w-full bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs outline-none focus:border-primary/40 transition-all h-9 text-foreground px-3"
            value={draft.timing}
            onChange={(event) => updateDraft((prev) => ({ ...prev, timing: event.target.value }))}
            onBlur={() => onSave(blockId, draft)}
            disabled={disabled}
          />
        </div>
        <div className="space-y-1.5">
          ...
          <input
            className="w-full bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-[6px] text-xs outline-none focus:border-primary/40 transition-all h-9 text-foreground px-3"
            value={draft.avoid}
            onChange={(event) => updateDraft((prev) => ({ ...prev, avoid: event.target.value }))}
            onBlur={() => onSave(blockId, draft)}
            disabled={disabled}
          />
        </div>
...
SoundEffectPromptEditor.displayName = "SoundEffectPromptEditor";
...
const parseAnimationPrompt = (raw: string | null | undefined): { prompt?: string; motion?: string; camera?: string; duration_hint?: string } | null => {
  if (!raw) return null;
  try {
    return JSON.parse(raw) as { prompt?: string; motion?: string; camera?: string; duration_hint?: string };
  } catch {
    return null;
  }
};

const parseDirectionNotes = (raw: string | null | undefined): { notes?: string } | null => {
  if (!raw) return null;
  try {
    return JSON.parse(raw) as { notes?: string };
  } catch {
    return null;
  }
};

const parseSoundEffectPrompt = (raw: string | null | undefined): { prompt?: string; timing?: string; avoid?: string } | null => {
  if (!raw) return null;
  try {
    return JSON.parse(raw) as { prompt?: string; timing?: string; avoid?: string };
  } catch {
    return null;
  }
};
...
const CourseVideoEditor: React.FC<CourseVideoEditorProps> = ({
  lessonId,
  dispatchAgentId,
  lessonTitle,
  moduleTitle,
    ...
    apiGet<LegacyBlock[]>(`/lesson-versions/${selectedVersionId}/blocks`)
      .then((items) => {
        const mapped = items.map((block) => {
          ...
          const animationPrompt = parseAnimationPrompt(block.animationPromptJson);
          const directionNotes = parseDirectionNotes(block.directionNotesJson);
          const soundEffectPrompt = parseSoundEffectPrompt(block.soundEffectPromptJson);
          return {
            id: block.id,
            ...
            animationPrompt: {
              prompt: animationPrompt?.prompt ?? '',
              motion: animationPrompt?.motion ?? '',
              camera: animationPrompt?.camera ?? '',
              durationHint: animationPrompt?.duration_hint ?? ''
            },
            directionNotes: {
              notes: directionNotes?.notes ?? ''
            },
            soundEffectPrompt: {
              prompt: soundEffectPrompt?.prompt ?? '',
              timing: soundEffectPrompt?.timing ?? '',
              avoid: soundEffectPrompt?.avoid ?? ''
            }
          } as LessonBlock;
        });
        setBlocks(mapped);
  ...
  const saveAnimationPrompt = useCallback(async (blockId: string, value: LessonBlock["animationPrompt"]) => {
    try {
      const payload = {
        prompt: value.prompt,
        motion: value.motion,
        camera: value.camera,
        duration_hint: value.durationHint
      };
      await apiPatch(`/blocks/${blockId}`, { animationPrompt: payload });
      updateBlock(blockId, { animationPrompt: value });
    } catch (err) {
      console.error(err);
      setError((err as Error).message ?? 'Failed to save animation prompt.');
    }
  }, [updateBlock]);

  const saveDirectionNotes = useCallback(async (blockId: string, value: LessonBlock["directionNotes"]) => {
    try {
      await apiPatch(`/blocks/${blockId}`, { directionNotes: { notes: value.notes } });
      updateBlock(blockId, { directionNotes: value });
    } catch (err) {
      console.error(err);
      setError((err as Error).message ?? 'Failed to save direction notes.');
    }
  }, [updateBlock]);

  const saveSoundEffectPrompt = useCallback(async (blockId: string, value: LessonBlock["soundEffectPrompt"]) => {
    try {
      const payload = {
        prompt: value.prompt,
        timing: value.timing,
      ...
      await apiPatch(`/blocks/${blockId}`, { soundEffectPrompt: payload });
      updateBlock(blockId, { soundEffectPrompt: value });
    } catch (err) {
      console.error(err);
      setError((err as Error).message ?? 'Failed to save sound effect prompt.');
    }
  }, [updateBlock]);
  ...
  const mergeLegacyBlock = (legacy: LegacyBlock) => {
    ...
    const animationPrompt = parseAnimationPrompt(legacy.animationPromptJson);
    const directionNotes = parseDirectionNotes(legacy.directionNotesJson);
    const soundEffectPrompt = parseSoundEffectPrompt(legacy.soundEffectPromptJson);
    setBlocks((prev) => {
  ...
  const contentValidationAlerts = [
    ...
    ...missingAnimationPromptBlocks.map((block) => ({
      key: `missing-animation-prompt-${block.id}`,
      blockId: block.id,
      message: `${block.number}: falta prompt de animação.`
    }))
  ];
            ...
            >
              ...
              <div className="bg-[hsl(var(--editor-surface))] border border-[hsl(var(--editor-border))] rounded-[5px] flex gap-6 overflow-hidden min-h-[600px]">
                ...
                <section className="space-y-3 pt-2 border-t border-dashed border-[hsl(var(--editor-border))]">
                  ...
                  />
                  <AnimationPromptEditor
                    blockId={block.id}
                    value={block.animationPrompt}
                    onSave={saveAnimationPrompt}
                    disabled={Boolean(generatingStates[block.id]?.text || isTextGenerationBusy || isGeneratingFinalVideo)}
                    invalid={blockMissingAnimationPrompt}
                  />
                  <DirectionNotesEditor
                    blockId={block.id}
                    value={block.directionNotes}
                    onSave={saveDirectionNotes}
                    disabled={Boolean(generatingStates[block.id]?.text || isTextGenerationBusy || isGeneratingFinalVideo)}
                  />
                  <SoundEffectPromptEditor
                    blockId={block.id}
                    value={block.soundEffectPrompt}
                    onSave={saveSoundEffectPrompt}
                    disabled={Boolean(generatingStates[block.id]?.text || isTextGenerationBusy || isGeneratingFinalVideo)}
                  />
                </section>
                </div>
