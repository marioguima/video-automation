...
const Security: React.FC<SecurityProps> = ({ currentUser }) => {
  ...
  const handleUpdatePassword = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentPassword.length < 8 || newPassword.length < 8) {
      setPasswordError('Passwords must have at least 8 characters.');
      return;
    }
    ...
    <div className="h-full overflow-y-auto custom-scrollbar bg-background">
      <div className="max-w-4xl mx-auto p-6 md:p-10 pb-24">
        ...
        <div className="space-y-8">
          ...
          {/* 1. Password Change */}
          <form onSubmit={handleUpdatePassword} className="bg-card border border-border rounded-[5px] shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3 bg-[hsl(var(--secondary))]/60">
            ...
            <div className="p-8">
              ...
              {(passwordError || passwordStatus) && (
                <div
                  className={`mt-5 rounded-[5px] border px-3 py-2 text-xs font-medium ${
                    passwordError
              ...
              <div className="mt-6 flex justify-end">
                <button
                  ...
                  className="flex items-center gap-2 px-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[5px] text-xs font-bold uppercase tracking-widest hover:bg-slate-800 dark:hover:bg-slate-200 transition-all h-9 disabled:opacity-70 disabled:cursor-not-allowed"
                >
