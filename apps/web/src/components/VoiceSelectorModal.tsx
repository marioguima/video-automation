...
const VoiceSelectorModal: React.FC<VoiceSelectorModalProps> = ({ 
  ...
  useEffect(() => {
    if (isOpen) {
      if (providedVoices && providedVoices.length > 0) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 lg:p-12">
      ...
      <div className="relative bg-card w-full max-w-6xl h-full max-h-[850px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-border">
          ...
          <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors h-9">
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Sidebar Filters */}
          <aside className="w-64 border-r border-border p-6 flex flex-col gap-8 bg-[hsl(var(--secondary))]/60 overflow-y-auto">
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-3 border rounded-lg text-xs outline-none focus:border-primary/40 transition-all h-9 bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                  value={searchQuery}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center ${voice.gender === 'male' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
                          <User size={16} />
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate">{voice.name}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight truncate">{voice.age_group} • {voice.gender}</p>
                        </div>
                      </div>
                      <button 
        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-900 border-t border-border">
          <div className="max-w-5xl mx-auto flex items-center gap-6">
              ...
              <div className="flex gap-4">
                <div className="relative flex-1">
                  <input 
                    ...
                    className="w-full h-9 bg-[hsl(var(--editor-input))] border border-[hsl(var(--editor-input-border))] rounded-xl pl-4 pr-12 text-sm text-foreground outline-none focus:border-primary/40 transition-all"
                    value={testText}
                  ...
                  <button 
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-600/10 rounded-lg disabled:opacity-30 transition-all h-9"
                  >
                ...
                {testAudioUrl && (
                  <div className="flex items-center gap-2 animate-in slide-in-from-right-2">
                    <button 
                      className="h-9 px-4 bg-indigo-600 text-white rounded-xl flex items-center gap-2 font-bold text-xs hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-600/20"
                      onClick={() => {
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-border">
              <button 
                onClick={onClose}
                className="px-6 text-sm font-bold text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 h-9"
              >
                Cancel
              </button>
              <button 
                ...
                className="px-8 h-9 bg-orange-600 text-white rounded-xl font-bold text-sm shadow-xl shadow-orange-600/20 hover:bg-orange-700 transition-all active:scale-95 flex items-center gap-2 disabled:opacity-50"
              >
                <Check size={18} />
                Confirm Voice
              </button>
...
const FilterButton: React.FC<{ label: string; active: boolean; onClick: () => void }> = ({ label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center justify-between px-3 rounded-lg text-xs font-bold transition-all ${ active ? 'bg-orange-600 text-white shadow-md' : 'text-muted-foreground hover:bg-slate-100 dark:hover:bg-slate-800' } h-9`}
  >
    {label}
