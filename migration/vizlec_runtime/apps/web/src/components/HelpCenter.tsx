    ...
    >
      <div className="flex items-start gap-4">
        <div className="flex-1">
          {block.type === 'title' ? (
             <h3 className={`text-lg font-bold transition-colors ${isPlaying ? 'text-orange-800 dark:text-orange-200' : 'text-foreground'}`}>
                {block.text}
             </h3>
          ) : (
        ...
        `}>
           <button className={`w-8 h-8 flex items-center justify-center transition-all ${isPlaying ? 'text-orange-600' : 'text-slate-400 hover:text-orange-600 hover:scale-110' }`}>
              {isPlaying && !isPaused ? (
  ...
  return (
    <div className="h-full flex flex-col bg-background relative">
      ...
      <div className="bg-card border-b border-border p-6 flex-shrink-0">
        <div className={`flex items-center justify-between ${layoutMode === 'center' || activeTab === 'support' ? 'max-w-5xl mx-auto' : ''}`}>
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Get support, view your tickets, and browse FAQs.</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
              <button 
                ...
                className={`px-4 h-9 rounded-md text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'support' ? 'bg-white dark:bg-slate-700 text-orange-600 shadow-sm' : 'text-muted-foreground'}`}
              >
                My Tickets
              </button>
              <button 
                onClick={() => setActiveTab('faq')}
                className={`px-4 h-9 rounded-md text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'faq' ? 'bg-white dark:bg-slate-700 text-orange-600 shadow-sm' : 'text-muted-foreground'}`}
              >
                Knowledge Base
              </button>
            </div>
        ...
        <div className={`h-full w-full flex flex-col ${getContainerClass()} transition-all duration-300`}>
              ...
              <div className="flex items-center justify-between mb-6">
                 <div className="relative">
                    ...
                    <input 
                      ...
                      className="pl-10 pr-4 border rounded-lg text-sm outline-none focus:border-primary/40 transition-all w-64 h-9 bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                      value={ticketSearch}
                 ...
                 <button 
                   ...
                   className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-4 h-9 rounded-lg text-sm font-bold uppercase tracking-wide transition-all shadow-md shadow-orange-600/20"
                 >
                ...
                <div className="space-y-4">
                  {filteredTickets.map(ticket => (
                    <div 
                      ...
                      className="bg-card border border-border rounded-xl p-5 hover:border-orange-500/50 hover:shadow-md transition-all cursor-pointer group"
                    >
          ...
          {activeTab === 'support' && currentView === 'create-ticket' && (
             <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                ...
                <div className="bg-card border border-border rounded-xl p-8 max-w-2xl mx-auto shadow-sm">
                   <h2 className="text-xl font-bold text-foreground mb-6">Create New Support Ticket</h2>
                   <form onSubmit={handleCreateSubmit} className="space-y-6">
                      <div className="space-y-2">
                         <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Subject</label>
                         <input 
                           required
                           className="w-full border rounded-lg text-sm outline-none focus:border-primary/40 transition-all h-9 bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground px-3"
                      ...
                      <div className="space-y-2">
                         ...
                         <textarea 
                           required
                           className="w-full border rounded-lg p-3 text-sm outline-none focus:border-primary/40 transition-all h-40 resize-none bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                      ...
                      <div className="pt-4 flex justify-end gap-3">
                         <button 
                           type="button"
                           onClick={() => setCurrentView('ticket-list')}
                           className="px-6 h-9 text-xs font-bold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-all uppercase tracking-widest"
                         >
                           Cancel
                         </button>
                         <button 
                           type="submit"
                           className="bg-orange-600 hover:bg-orange-700 text-white px-8 rounded-lg text-xs font-bold uppercase tracking-widest shadow-lg shadow-orange-600/20 transition-all active:scale-95 h-9"
                         >
                           Submit Ticket
                         </button>
                      </div>
          ...
          {activeTab === 'support' && currentView === 'ticket-detail' && selectedTicket && (
              <div className="flex flex-col h-full overflow-hidden">
                  <div className="p-6 border-b-2 border-dashed border-border flex items-center justify-between flex-shrink-0">
                      <div className="flex items-center gap-4">
                          <button 
                             onClick={() => setCurrentView('ticket-list')}
                      ...
                      <div className="max-w-3xl mx-auto relative">
                          ...
                          {selectedTicket.messages.map((msg, idx) => (
                              <div key={msg.id} className="relative pl-14 pb-10 group">
                                  ...
                                  <div className={`bg-card border ${msg.senderRole === 'support' ? 'border-orange-200 dark:border-orange-900/30 shadow-orange-500/5' : 'border-border'} rounded-xl p-6 shadow-sm`}>
                  ...
                  <div className="p-6 pt-0 bg-background">
                      <div className="max-w-3xl mx-auto">
                          <div className="relative">
                              <textarea 
                                  className="w-full border rounded-xl p-4 pr-32 text-sm outline-none focus:border-primary/40 transition-all resize-none h-32 shadow-sm bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                                  placeholder="Type your reply here..."
                                  value={newMessage}
                                  onChange={(e) => setNewMessage(e.target.value)}
                              />
                              <div className="absolute bottom-4 right-4 flex items-center gap-2">
                                  <button className="p-2 text-slate-400 hover:text-orange-600 transition-colors h-9">
                                      <Paperclip size={18} />
                                  </button>
                                  <button 
                                    onClick={handleSendMessage}
                                    disabled={!newMessage.trim()}
                                    className="bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 rounded-lg text-sm font-bold flex items-center gap-2 transition-all shadow-md h-9"
                                  >
                                      <Send size={16} /> Reply
                                  </button>
                              </div>
          ...
          {activeTab === 'faq' && (
             <div className="flex flex-1 overflow-hidden h-full">
                {/* FAQ Sidebar */}
                <aside className="w-64 border-r border-border overflow-y-auto custom-scrollbar hidden md:block bg-card p-6 flex-shrink-0">
                    <div className="relative mb-6">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                        <input 
                        type="text"
                        placeholder="Search topics..."
                        className="w-full pl-9 pr-3 border focus:border-primary/40 rounded-lg outline-none transition-all text-xs h-9 bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                        value={searchQuery}
                    ...
                    {filteredCategories.map(category => (
                        ...
                        <div className="space-y-1">
                            ...
                            <button
                               ...
                               className={`w-full text-left px-3 h-9 rounded-lg text-xs font-medium transition-colors flex items-center justify-between group ${
                ...
                <main className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-background">
                    ...
                    `}>
                        ...
                        <h2 className="text-3xl font-bold text-foreground mb-8 border-b border-border pb-6">
                            {activeTopic.title}
                        </h2>
      ...
      {activeTab === 'faq' && currentSpokenText && (
        <div className={`absolute bottom-0 left-0 right-0 bg-card border-t border-border p-4 shadow-2xl z-50 animate-in slide-in-from-bottom-5`}>
           <div className={`flex items-center gap-6 ${layoutMode === 'full-width' ? 'max-w-full px-6' : 'max-w-5xl mx-auto'}`}>
              ...
              <div className="items-center gap-4 border-l border-border pl-6 hidden sm:flex">
                 <div className="flex flex-col items-end">
                    <span className="text-xs font-bold text-foreground">John Hunt</span>
                    <span className="text-[10px] text-slate-400">AI Narrator</span>
                 </div>
                 <button 
                  onClick={handleGlobalStop}
                  className="text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors h-9"
                  title="Stop"
                 >
                    <XCircle size={24} />
                 </button>
