...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, currentUser, onLogout }) => {
  ...
  const coursesSectionViews = new Set<ViewType>([
    'courses',
    'course-editor',
    'modules',
    'module-editor',
  ...
  const getButtonClass = (isActive: boolean) => `
    w-full justify-start gap-3 h-12 font-semibold border border-transparent transition-all duration-200
  ...
  return (
    <aside 
      className={`
        ${isCollapsed ? 'w-20' : 'w-20 lg:w-60'} 
        border-r border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar))] text-[hsl(var(--sidebar-foreground))] flex flex-col h-screen sticky top-0 transition-all duration-300 z-50 group/sidebar
      `}
    >
      <Button
        variant="outline"
        ...
        className="absolute -right-3 top-[66px] z-50 h-7 w-7 rounded-full shadow-sm hidden lg:flex bg-[hsl(var(--header))] border border-[hsl(var(--header-border))] text-muted-foreground hover:text-foreground hover:bg-[hsl(var(--header))] transition-colors"
      ...
      <div className="px-4 pb-4 space-y-2">
        <Button
          ...
          className={`${getButtonClass(currentView === 'help')} opacity-70 hover:opacity-100`}
        >
      ...
      {/* Profile Section */}
      <div className="p-4 relative border-t border-border" ref={profileMenuRef}>
        {isProfileMenuOpen && (
          <div className="absolute bottom-full left-4 mb-3 w-[280px] bg-popover text-popover-foreground border border-border rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 zoom-in-95 duration-200 z-[60]">
             <div className="p-2 border-b border-border">
                ...
                <div className="space-y-0.5">
                  ...
                  <DropdownItem icon={Settings} label="Settings" onClick={() => { setView('settings'); setIsProfileMenuOpen(false); }} />
                </div>
             </div>

             <div className="p-2 border-b border-border">
                <div className="bg-muted/50 rounded-lg p-3 border border-border relative overflow-hidden group">
        ...
        <Button
          variant="ghost"
          className={`w-full justify-start p-2 h-auto rounded-xl hover:bg-accent ${isCollapsed ? 'justify-center' : ''}`}
        ...
        >
          ...
          <div className={`flex-1 overflow-hidden text-left ml-3 transition-all duration-200 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'hidden lg:block'}`}>
            <p className="text-sm font-semibold truncate text-foreground/80">{currentUser?.name ?? 'User'}</p>
            <p className="text-xs font-medium text-muted-foreground truncate">
              {(currentUser?.role ?? 'member').toUpperCase()}
            </p>
          </div>
          <ChevronDown 
            className={`w-4 h-4 text-muted-foreground ml-auto transition-transform duration-200 ${isProfileMenuOpen ? 'rotate-180' : ''} ${isCollapsed ? 'hidden' : 'hidden lg:block'}`} 
          />
        </Button>
      </div>
    </aside>
...
const DropdownItem = ({ icon: Icon, label, shortcut, onClick }: { icon: any, label: string, shortcut?: string, onClick?: () => void }) => (
  <button 
    onClick={onClick}
    className="w-full flex items-center justify-between px-3 text-muted-foreground hover:text-foreground hover:bg-accent rounded-md group transition-colors h-9"
  >
