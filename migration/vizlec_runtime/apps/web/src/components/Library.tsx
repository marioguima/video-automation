...
const Library: React.FC<LibraryProps> = ({ onImageClick }) => {
  ...
  const getLessonFiles = (lesson: LessonBlock) => {
    return [
      { id: 'f1', name: 'Raw_Asset.png', type: 'image', size: '1.2 MB', date: '2 mins ago', url: lesson.thumbLandscape || lesson.thumbnail || '/lesson-placeholder.svg' },
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" : "space-y-2"}>
          {COURSES.filter(c => c.title.toLowerCase().includes(filteredSearch)).map(course => (
            <FolderCard 
              ...
              viewMode={viewMode}
              thumbnail={course.thumbLandscape || course.thumbnail || '/course-placeholder.svg'}
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" : "space-y-2"}>
          {lessons.filter(l => l.title.toLowerCase().includes(filteredSearch)).map(lesson => (
            <FolderCard 
              ...
              viewMode={viewMode}
              thumbnail={lesson.thumbLandscape || lesson.thumbnail || '/lesson-placeholder.svg'}
              onClick={() => navigateTo(lesson, 'files')}
              onThumbnailClick={() => onImageClick?.(lesson.thumbLandscape || lesson.thumbnail || '/lesson-placeholder.svg')}
            />
          ))}
        </div>
      );
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Browse and manage generated assets across your courses.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 bg-slate-100 border focus:border-primary/30 rounded-lg text-sm outline-none w-64 transition-all h-9 bg-[hsl(var(--editor-input))] border-[hsl(var(--editor-input-border))] text-foreground"
                  value={searchQuery}
      ...
      <div className="max-w-7xl w-full mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          {depth !== 'courses' && (
            <button 
              onClick={goBack}
              className="flex items-center gap-2 px-3 text-slate-500 hover:text-orange-600 transition-colors font-bold text-xs uppercase tracking-widest h-9"
            >
              <ArrowLeft size={16} />
              Back
            </button>
...
const FolderCard: React.FC<{ 
  ...
  if (viewMode === 'list') {
    return (
      <div 
        onClick={onClick}
        className="flex items-center gap-4 p-3 bg-card border border-border rounded-lg hover:border-orange-500/50 transition-all cursor-pointer group"
      >
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-card border border-border rounded-xl p-5 hover:border-orange-500/50 hover:shadow-lg transition-all cursor-pointer group text-center flex flex-col items-center"
    >
      <div 
...
const FileCard: React.FC<{ file: any; viewMode: 'grid' | 'list', onImageClick?: (url: string) => void }> = ({ file, viewMode, onImageClick }) => {
  ...
  if (viewMode === 'list') {
    return (
      <div className="flex items-center gap-4 p-3 bg-card border border-border rounded-lg hover:border-orange-500/50 transition-all group">
        <div 
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="p-2 text-slate-400 hover:text-orange-600 transition-colors h-9"><Download size={16} /></button>
          <button className="p-2 text-slate-400 hover:text-orange-600 transition-colors h-9"><MoreVertical size={16} /></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden hover:border-orange-500/50 hover:shadow-lg transition-all group">
      <div 
...
const FilterButton: React.FC<{ active: boolean; label: string; onClick: () => void }> = ({ active, label, onClick }) => (
  <button 
    onClick={onClick}
    className={`px-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${ active ? 'bg-orange-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-orange-600 dark:text-slate-400' } h-9`}
  >
    {label}
  </button>
);
